...
void D3D11RenderingContext::CreateSwapchain(Window* window)
{
    BT_LOG_TRACE(Renderer, "Creating a swapchain targeting the window '%s'...", window->GetTitle().c_str());
    ...
    swapchain_desc.OutputWindow = (HWND)window->GetNativeHandle();
    swapchain_desc.Windowed = (window->GetMode() == EWindowMode::Windowed);
